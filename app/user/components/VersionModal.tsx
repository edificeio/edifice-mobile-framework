...
export default function VersionModal(props: { version: string, visibility: boolean, mandatory: boolean, onCancel: () => void, onSubmit: () => void }) {
    const { mandatory, visibility, version, onCancel, onSubmit } = props;
    if (!visibility) {
        return null;
    }
    if (mandatory) {
        return <ModalBox backdropOpacity={0.5} isVisible={visibility}>
            <ModalContent>
                <LightP>{I18n.t("common-VersionTitle")}</LightP>
                <Paragraph style={{ textAlign: "center", paddingTop: 12 }}>{I18n.t("common-VersionContentMandatory", { version })}</Paragraph>
                <ButtonsOkOnly
                    onValid={onSubmit}
