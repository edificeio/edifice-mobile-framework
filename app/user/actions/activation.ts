...
export function activationAccount(model: IActivationModel) {
    return async (dispatch, getState) => {
        try {
            ...
            if (!theme) {
                console.warn("[User][Activation] activationAccount -> theme was not found:", Conf.currentPlatform.themes);
            }
            ...
            if (res.headers.get("content-type").indexOf("application/json") !== -1) {// checking response header
