...
export const CREATE_THREAD_CONVERSATION = (
  ...
  return {
    ...
    threads: [
      ...
      ...state.threads.filter(t => t.thread_id !== "temp")
    ],
    pickedUsers: [],
...
export const CONVERSATION_SENT = (state, action) => {
  ...
  const newParentThread = {
    ...parentThread,
    messages: [
      { ...action.data, thread_id: threadId },
  ...
  return {
    ...
    threads: [
      ...state.threads.filter(p => p !== parentThread),
      newParentThread
