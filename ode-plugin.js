...
async function _updateAppIds(ode) {
  ...
  pbxproj = pbxproj.replace(/(PRODUCT_BUNDLE_IDENTIFIER\s*")(.*)("/, '$1' + appidIOS + '$3');
...
async function overrideSwitchTo(name, skipRestore, acceptAll, gitUri, gitBranch, gitUser, gitPwd) {
  ...
  //
  if (skipRestore) {
    //for example dont restore from appcenter
  ...
  if (filePaths.length) {
    ...
    for (let i = 0; i < filePaths.length; i++) {
      ...
      if (status == '??') {
        ...
        await exec('git reset HEAD ' + filePath).then(function() {
...
async function overrideBackup() {
  ...
  if (filePaths.length) {
    ...
    for (let i = 0; i < filePaths.length; i++) {
      ...
      fileListAbsolute = fileListAbsolute.filter(function(value) {
        return value != overrideFilePath;
      });
      //
