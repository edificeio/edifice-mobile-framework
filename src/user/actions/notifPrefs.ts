...
export const actionTypeSetNotifPrefs = userConfig.createActionType('NOTIFICATIONS_PREFS_SET');
...
export function DEPRECATED_loadNotificationPrefs() {
  return async (dispatch: any, getState: any) => {
    ...
    const userNotifPrefsConfig = Object.values(rawUserTimelinePrefs.config).reduce((acc, el: any) => {
    ...
    const rawDefaultNotifsPrefsConfig = await fetchJSONWithCache('/timeline/notifications-defaults');
    const defaultNotifsPrefsConfig = !rawDefaultNotifsPrefsConfig
      ? {}
      : rawDefaultNotifsPrefsConfig.reduce((acc: any, el: any) => {
          if (includeNotifKeys.includes(el.key as never)) {
            acc[el.key] = { ...el, 'push-notif': el['push-notif'] || false }; // In the eventual case of push-notifs is not defined for this key, we consider `false`.
          }
          return acc;
...
export const includeNotifKeysByApp = [
  ...
  {
    appName: 'schoolbook',
    notifKeys: ['schoolbook.publish', 'schoolbook.word-resend', 'schoolbook.acknowledge'],
  },
  {
    appName: 'blog',
