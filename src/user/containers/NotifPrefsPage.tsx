...
export const NotifPrefsPageNavigationOptions = ({ navigation }: { navigation: NavigationScreenProp<object> }) =>
...
export type INotifPrefsPageProps = INotifPrefsPageDataProps & INotifPrefsPageEventProps & INotifPrefsPageOtherProps;

// tslint:disable-next-line:max-classes-per-file
export class NotifPrefsPage extends React.PureComponent<INotifPrefsPageProps, undefined> {
  ...
  public render() {
    ...
    const notifPrefsLines = [...new Set(Object.values(this.props.notificationPrefs).map(pref => pref['type'].toLowerCase()))];
          ...
          {notifPrefsLines
            ...
            .map((appName: string) => (
              <NotifPrefLine
                ...
                value={
                  Object.values(this.props.notificationPrefs).find(pref => pref['type'].toLowerCase() === appName)['push-notif']
                }
...
const NotifPrefsPageConnected = connect(
  (state: any) => {
    ...
    includeNotifKeys.map(prefName => {
      if (state.user.auth.notificationPrefs[prefName]) sortedNotifPrefs[prefName] = state.user.auth.notificationPrefs[prefName];
    });
  ...
  dispatch => ({
    ...
    onTogglePref: (notification, pref, notificationPrefs) =>
      dispatch(action_toggleNotifPrefsByApp(notification, pref, notificationPrefs) as any),
  }),
)(NotifPrefsPage);
