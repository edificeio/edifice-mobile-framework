...
import { publishableBlogsActionTypes, IBlog, blogPublishActionTypes, IBlogList } from '~/timeline/state/publishableBlogs';
...
export const fetchPublishableBlogsAction =
  ...
  async (dispatch: Dispatch, getState: () => any) => {
    ...
    try {
      ...
      const filteredBlogs = (
        resourceRightFilter(allBlogs, 'org-entcore-blog-controllers-PostController|create', getState().user.info) as IBlog[]
...
export const blogPublishActions = createAsyncActionCreators<object>(blogPublishActionTypes);
export const publishBlogPostAction =
  (blog: IBlog, title: string, content: string, uploadedBlogPostDocuments?: any) =>
  async (dispatch: Dispatch & ThunkDispatch<any, void, AnyAction>, getState: () => any) => {
    const api = `${DEPRECATED_getCurrentPlatform()!.url}/blog/post/${blog._id}`;
    ...
    try {
      ...
      if (uploadedBlogPostDocuments) {
        ...
        const images = blogPostUploads
          .map(blogPostUpload => `<img src="${blogPostUpload.url}?thumbnail=2600x0" class="">`)
          .join('');
        ...
        notifierShowAction({
          ...
          text: I18n.t(
            hasSubmitRight || hasPublishRight
              ? blog['publish-type'] === 'IMMEDIATE'
                ? 'createPost-publishSuccess'
                : 'createPost-submitSuccess'
