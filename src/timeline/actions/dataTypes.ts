...
import { IMediaModel } from '~/timeline/reducer';

let loadingState = 'idle';
const awaiters = [];
export let schoolbooks: any[] = [];
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (getSessionInfo().type.indexOf('Student') !== -1) {
      try {
        ...
        let messages: any[] = await fetchJSONWithCache(`/schoolbook/list/0/${getSessionInfo().userId}`);
        messages = messages || [];
      ...
      try {
        ...
        for (const childId of Object.values((getSessionInfo() as any).childrenIds)) {
          if (!childId) continue;
          let messages = await fetchJSONWithCache(`/schoolbook/list/0/${childId}`);
          messages = messages || [];
