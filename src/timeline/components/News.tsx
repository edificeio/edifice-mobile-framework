...
import { INewsModel, IMediaModel } from '~/timeline/reducer';
...
export class News extends React.PureComponent<INewsProps, INewsState> {
  ...
  getMediaPreview() {
    ...
    if (firstItem) {
      if (firstItem.type === 'attachment') {
        const mediaAttachments: IMediaModel[] = [];
        ...
        return <AttachmentGroup attachments={attachments as IRemoteAttachment[]} />;
      } else if (firstItem.type === 'image') {
        const images: IMediaModel[] = [];
        for (const mediaItem of media) {
  ...
  hasAdditionalContent() {
    ...
    if (message && message.endsWith('...')) {
      // the message is too long and is cropped
      return true;
    }
    if (firstItem) {
      if (firstItem.type === 'image' || firstItem.type === 'attachment') {
        let mediaCount = 0;
        for (const mediaItem of media) {
          if (mediaItem.type !== firstItem.type) {
            // even if the first batch of images/attachments has less than 4 items, there might be additional medias
            return true;
          }
          mediaCount++;
        }
        ...
        <TouchCard onPress={() => this.open(false)} style={{ width: '100%' }}>
          <View>
            ...
            {message && /\S/.test(message) ? (
              <Text style={{ color: CommonStyles.textColor, marginBottom: firstItem ? 10 : undefined }}>{message}</Text>
            ) : null}
          ...
          {this.hasAdditionalContent() ? (
            <>
              ...
              />
              <TouchableOpacity style={{ flexDirection: 'row', alignSelf: 'center' }} onPress={() => this.open(true)}>
                <A style={{ marginRight: 10 }}>{I18n.t('seeMore')}</A>
                <Icon name="arrow_down" color={CommonStyles.actionColor} style={{ transform: [{ rotate: '270deg' }] }} />
              </TouchableOpacity>
            </>
          ) : null}
        </TouchCard>
