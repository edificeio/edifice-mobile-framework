...
export const HomeScreen = withSession<HomeScreenProps>(({ navigation, session }) => {
  ...
  React.useEffect(() => {
    ...
    const createButton = (props: Parameters<typeof headerAction>[1]) => {
      ...
      action.element = (
        <PopupMenu actions={workflows} testID="home-add-menu">
          {action.element}
        </PopupMenu>
      );
      return action;
    ...
    navigation.setOptions({
      headerRight: props => createButton(props).element,
      unstable_headerRightItems: props => [createButton(props)] as NativeStackHeaderItem[],
    });
