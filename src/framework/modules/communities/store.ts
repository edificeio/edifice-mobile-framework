...
export interface CommunitiesStore {
  allCommunities: (CommunityResponseDtoWithThumbnails | typeof LOADING_ITEM_DATA)[];
...
export type CommunitiesAction<T> = T extends typeof communitiesActionTypes.LOAD_ALL_COMMUNITIES_PAGE
  ? {
      type: typeof communitiesActionTypes.LOAD_ALL_COMMUNITIES_PAGE;
      payload: { newData: { items: CommunityResponseDtoWithThumbnails[]; from: number; total: number }; reloadAll?: boolean };
    }
  : T extends typeof communitiesActionTypes.LOAD_PENDING_COMMUNITIES_PAGE
    ? {
        type: typeof communitiesActionTypes.LOAD_PENDING_COMMUNITIES_PAGE;
...
export const communitiesActions = {
  loadAllCommunitiesPage: (
    newData: { items: CommunityResponseDtoWithThumbnails[]; from: number; total: number },
    reloadAll = false,
  ) => ({
