...
const getCollectClient = () => sessionApi(COLLECT_API, CollectionClient as any) as unknown as CollectionClient; // bug with CollectionClient typings
...
export const getCollectionsByCollectId = async (
  ...
  return {
    adminCollections: new Map(collections.map(collect => [collect.id, collect])),
    memberSubmissions: new Map(submissions.map(submission => [submission.collectionId, submission])),
  };
};

export const getOpenCollectsCount = async (collectIds: number[], userRole?: MembershipRole): Promise<number> => {
  if (!collectIds.length) return 0;

  const collectClient = getCollectClient();
  const queryParams = { ids: collectIds };

  if (userRole === MembershipRole.ADMIN) {
    const { total } = await collectClient.getCollectionsCount(queryParams);
    return total;
  }

  const { pending } = await collectClient.getCollectionsToSubmitCount(queryParams);
  return pending;
};
