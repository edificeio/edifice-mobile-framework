...
const CompetencesHomeScreen = (props: CompetencesHomeScreenPrivateProps) => {
  ...
  const openAssessment = (assessment: IDevoir) => {
    ...
    const studentClass = userType === AccountTyoe.Student ? classes?.[0] : userChildren?.find(c => c.id === childId)?.classId;
  ...
  return (
    <PageView>
      {props.userType === AccountTyoe.Relative ? <ChildPicker contentContainerStyle={styles.childPickerContentContainer} /> : null}
      {renderPage()}
    </PageView>
  );
};
