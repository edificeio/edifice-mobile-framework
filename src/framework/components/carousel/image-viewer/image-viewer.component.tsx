...
export default class ImageViewer extends React.Component<Props, State> {
  ...
  public componentDidUpdate(prevProps: Props, prevState: State) {
    if (prevProps.index !== this.props.index) {
      // 立刻预加载要看的图
      this.loadImageWindow(this.props.index || 0);
  ...
  public init(nextProps: Props) {
    ...
    this.setState(
      ...
      () => {
        // 立刻预加载要看的图
        this.loadImageWindow(nextProps.index || 0);
  ...
  public loadImage(index: number) {
    ...
    if (image.width && image.height) {
      if (image.url && this.props.enablePreload && imageLoaded === false) {
    ...
    const getSizeSuccess = (width: number, height: number) => {
