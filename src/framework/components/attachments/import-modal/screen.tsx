...
import { AttachmentsImportScreenProps } from './types';
...
export const computeNavBar = ({
  navigation,
  route,
}: NativeStackScreenProps<IModalsNavigationParams, typeof ModalsRouteNames.AttachmentsImport>): NativeStackNavigationOptions => ({
  presentation: 'modal',
  ...navBarOptions({
    navigation,
    route,
...
interface UploadFile {
  localFile: LocalFile | SyncedFileWithId;
  status: UploadStatus;
  error?: string;
}
...
export default function AttachmentsImportScreen(props: AttachmentsImportScreenProps) {
  ...
  const updateFileStatus = React.useCallback(
    ({ error, file, status }: { file: UploadFile; status: UploadStatus; error?: string }) => {
      file.status = status;
  ...
  const uploadFile = React.useCallback(
    (file: UploadFile, index: number) => {
      ...
      if (!session) {
        updateFileStatus({ file, status: UploadStatus.KO });
        return;
      }
      uploadingTasksRef.current.add(file);
      updateFileStatus({ file, status: UploadStatus.PENDING });

      mailsService.attachments
        .add({ mailId: route.params.draftId }, file.localFile as LocalFile, session!)
        .then(resp => {
          updateFileStatus({ file: resp as SyncedFileWithId, status: UploadStatus.OK });
        })
        .catch(error => {
          console.error(`Import File Upload Failed: ${error}`);
          updateFileStatus({ error: textErrorUploadFile(error), file, status: UploadStatus.KO });
        })
        .finally(() => {
          uploadingTasksRef.current.delete(file);
        });
    },
    [session, updateFileStatus],
  );
