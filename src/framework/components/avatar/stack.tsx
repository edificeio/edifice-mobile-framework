  ...
  const avatarItemProps = React.useMemo(
    () =>
      displayableItems.map((item, index) => {
        ...
        const extraCount = total - displayableItems.length;
        if (index === displayableItems.length - 1 && extraCount > 0) {
          const displayedTotal = overlayI18nKey
            ? I18n.get(overlayI18nKey, { count: Math.min(extraCount, MAX_TOTAL) })
            : total > MAX_TOTAL
              ? I18n.get('avatar-count-out-of-reach', { count: MAX_TOTAL })
              : total;
    ...
    [_avatarItemProps, avatarItemStyle, displayableItems, overlayI18nKey, total],
  );
