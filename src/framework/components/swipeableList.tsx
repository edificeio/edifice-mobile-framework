  ...
  <ItemT extends { key: string }>( // need to write "extends" because we're in a tsx file
    ...
    const realRenderItem = React.useCallback(
      (info: ListRenderItemInfo<ItemT>, rowMap: RowMap<ItemT>) => {
        ...
        // no swipe actions ==> render row directly to avoid PanResponder interfering with taps.
        const hasActions = !!(actions?.left?.length || actions?.right?.length);
        if (!hasActions) {
          return <View>{renderItem(info, rowMap)}</View>;
        }
