...
export default function createReducer<StateType>(
  ...
  return (
    ...
    return reducerActionsHandlerMap[action.type]
      ? reducerActionsHandlerMap[action.type](state, action)
      : state;
  }
}
...
export function createSessionReducer<StateType>(
  ...
  return createReducer(initialState, {
    ...
    ...Object.fromEntries(
      sessionNamesUppercase.map(sessionName => [
        createEndSessionActionType(sessionName),
        () => initialState
      ])
    )
  });
