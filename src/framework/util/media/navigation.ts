...
interface MediaIntent<MediaType extends Media> {
  ...
  icon?: (media: MediaType) => IntentIcon;
}

const mediaIntents = [
  {
    condition(media) {
      return isImageMedia(media) || (isAttachmentMedia(media) && !!mimeCompare(media.mime, 'image/*'));
    },
    exec(media, audience) {
  ...
  {
    condition(media) {
      return isVideoMedia(media) || (isAttachmentMedia(media) && !!mimeCompare(media.mime, 'video/*'));
    },
    exec(media, audience) {
      openMediaPlayer({
        filetype: media.mime,
  ...
  {
    condition: media => isAudioMedia(media) || (isAttachmentMedia(media) && !!mimeCompare(media.mime, 'audio/*')),
    exec(media, audience) {
      openMediaPlayer({
        filetype: media.mime,
        referer: audience,
