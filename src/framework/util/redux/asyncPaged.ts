...
import { AsyncActionTypeKey, asyncActionTypeSuffixes, AsyncState, createAsyncActionTypes, createInitialAsyncState, ErrorAction, ReceiptAction } from "./async";
...
export const createAsyncPagedActionCreators: <DataType extends Array<any>>(actionTypes: AsyncPagedActionTypes) => AsyncPagedActionCreators<DataType> =
...
function _createAsyncPagedReducer<DataType extends Array<any>>(
    ...
    const asyncReducer = createReducerFunction<AsyncState<DataType>>(asyncInitialState, {
        ...
        [actionTypes.receipt]: (state, action: ReceiptPagedAction<DataType>) => ({
            ...
            data: [
                ...state.data.slice(0, pageSize * action.page),
                ...action.data,
                ...state.data.slice(pageSize * (action.page + 1))
            ]
        }),
