...
export default class HomeworkList extends React.PureComponent<any, any> {
  ...
  async componentDidMount() {
    const { childId, structureId, startDate, endDate } = this.state;
    const startDateString = moment(startDate).format("YYYY-MM-DD");
    const endDateString = moment(endDate).format("YYYY-MM-DD");
    this.props.fetchHomeworks(childId, structureId, startDateString, endDateString);
    this.props.fetchSessions(childId, startDateString, endDateString);
