...
export const actionTypesPast = asyncActionTypes('WORKSPACE_PAST');
...
export function pastAction(destinationId: string, selected: IItems<IItem>) {
  const api = '/workspace/documents/copy';
  ...
  return asyncActionFactory(`${api}/${root}`, { ids, parentId }, actionTypesPast, (data: any) => formatResults(data, parentId), {
    method: 'post',
  });
}
