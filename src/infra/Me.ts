...
export const savePreference = async (appName: string, newData) => {
  ...
  await signedFetch(`${DEPRECATED_getCurrentPlatform()!.url}/userbook/preference/${appName}`, {
...
export const preference = async (appName: string) => {
  if (!DEPRECATED_getCurrentPlatform()) throw new Error('must specify a platform');
  const appPrefs = (await signedFetchJson(`${DEPRECATED_getCurrentPlatform()!.url}/userbook/preference/${appName}`, {})) as {
    preference: any;
  };
